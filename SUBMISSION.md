...
| **Engineering** | **90 tests**, no network or credentials needed · one-command demo · **33 logged entries** with root causes |
...
One command runs it: `python run_demo.py` (trains, serves, replays 14,160 transactions through the real pipeline). **90 tests pass with no network and no credentials.**
