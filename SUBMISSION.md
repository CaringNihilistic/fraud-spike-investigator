...
| **False alarms** — entity-sharing negatives | **1 in 35 non-attack merchant-windows** (2.9%, 95% CI 0.1–14.9%) — a legitimate corporate buyer, on one seed. Three legitimate spikes tested, two sharing entities; the 6× flash sale flagged **0 / 5** |
| **False alarms** — growth negatives | **A second axis fires, and we left it visible.** A legitimate signup wave flags **0.6–13.9%** of its transactions (against an attack's 65–94%) and enters spike state on 2 of 5 seeds, restricting **₹2,914**. This is a different denominator — the row above never covered it |
...
| **Engineering** | **137 tests**, no network or credentials needed · one-command demo · **38 logged entries** with root causes |
...
One command runs it: `python run_demo.py` (trains, serves, replays 14,160 transactions through the real pipeline). **137 tests pass with no network and no credentials.**
...
- **The LLM investigates; it never decides.** Seven read-only tools, no tool exposes ground truth, all ₹ arithmetic in Python (LLM arithmetic is unauditable), audit log of every tool call. Live eval on Claude Haiku 4.5 over **de-labelled** data, 13 cases including 3 held-out on a fresh seed. **The numbers that do not depend on how we wrote the labels:** 100/100 evidence claims traceable to a tool output the agent actually received · escalates-when-unsure **5/5** *(only the 5 low-signal cases can fail this; the other 8 pass by construction)* · policy violations **0/13** · attacks let through **0 of the 7 attacks** · unsafe actions **0/13, and that number is vacuous** — it requires an attack still running at report time, and no case had one, so it could not have fired. **Three of those four were being reported over 13 when the number of cases that could actually fail them is 0, 7 and 5** ([failure 38](CLAUDE.md)) — corrected here rather than left to be found.
  **Action errors by direction: 4 over-cautious, 3 under-cautious.** We previously wrote that *every* action error was in the cautious direction. **That was wrong and is retracted** — counting them properly ([failure 37](CLAUDE.md)) found three cases where the agent recommended a weaker action than the label expected. **What actually contains the damage is the architecture, not the agent's judgment:** the policy engine restricted those merchants regardless, because the LLM is not in the decision path — the same property demonstrated under a real failure in the m2 incident below. Two supporting facts, with their limits stated: `attacks_let_through` is **0** and that is a real measurement (no attack was ever recommended `allow`); `unsafe_actions` is **0/13** but is **vacuous in this run** — it is defined as de-escalating *while an attack is still running*, and `attack_active_at_end` is 0 for **all 13** cases, so the metric had no opportunity to fire. That is a property of when the eval snapshots, not of what the agent did, and we would rather say so than bank the number.
  **Correct cause 8/13 (2/3 held-out)**, with two caveats already in the record: n=13 gives roughly a ±25-point interval, so **8/13 and 5/13 are not distinguishable**; and `correct_action` partly measures **label design**, because `expected_action` was authored before the baseline tool that let the agent see a finished attack as finished. Of the five cause misses: **2 are genuine reasoning failures** (it concluded account-takeover while its own evidence recorded no new device and no geo mismatch — the two defining ATO markers), 2 are the agent *declining to name a cause* at confidence 0.35–0.40 and escalating, and 1 is label-adjacent (card testing read as fraud ring; both attacks, both escalate). Reproduce: `python -m src.audit.agent_failure_taxonomy`.
...
**And we tested whether the detector generalises past the shapes we drew.** Our five attacks are five *topologies*, and the model had only ever been tested against the ones it trained on. So we rebuilt each attack with a different entity graph — card testing across **25 devices instead of 3**, the device farm across **10 instead of 1**, the ring sparse instead of dense — holding volume, burst window, amounts, ageing and fraud prevalence constant, and ran the **frozen pipeline** over 5 seeds with the known shapes as a positive control. **Four of the five unseen variants are strictly harder** (8–10× less entity concentration). Result: **known 22/25 detected, unseen 21/25** — but the mean flagged rate inside the attack fell **0.815 → 0.709**, and on the two families whose fan-out was removed it fell much further (card testing 0.758 → 0.436). **The merchant layer held while the transaction scorer lost confidence**, which is this product's thesis showing up as a measurement rather than a claim. Caveat we state first: account takeover fires only **2/5 even in its known form** here, so both its rows are weak evidence, and each variant degrades one signature rather than all of them.

**Then an audit found the gap none of our sweeps could see, and probing it cost us.** Every legitimate merchant we had except the flash sale was an *entity-sharing* case, so we had tested “shared infrastructure looks like fraud” exhaustively and “sudden legitimate growth looks like fraud” not at all. We pre-registered the criteria, committed them **before** running, and built the surge so `customer_age_days` was statistically degenerate against card testing (median 40.6 vs 43.2 days) — because the easy version of that test proves nothing. **It fires** — marginally, and the proportion carries more than the seed count: the surge flags **0.6–13.9%** of its transactions against an attack's **65–94%**, crossing the detector's bar on 2 of 5 seeds (*n=5 cannot separate 2/5 from 1/5*, the same caveat we apply to the n=13 agent eval), on a merchant with zero entity sharing, restricting **₹2,914** of legitimate revenue. The mechanism is specific: mean `is_first_seen_device` is **0.83 on the honest surge against 0.02 on the attack** — the legitimate wave maxes out that feature *harder than card testing does*. We did not patch it; patching training under deadline invalidates the frozen eval. It is failure-log 36 and a v2 item. **The other half came back clean:** a brand-new merchant with no history scores **+0.0006** against an identical warm twin, so there is no cold-start penalty and no cold-start rule is needed.
...
**The repository now mechanically refuses a stale headline number** — and only a number, which is a bound we measured rather than assumed: a claim carrying no figure slipped past it three times (failures 37–38). `python -m src.audit.verify_submission` exits non-zero when a documented number disagrees with the artifact that produced it, when a retracted phrasing appears unlabelled, or — the case no documentation check could see — when a shipped constant no longer equals the decision that adopted it. Validated by pointing it at our own docs from before the corrections below: it flags **10 problems**, including the retracted headline and the calibration figures an external auditor found by hand. Building it immediately caught a sixth stale table both audits had missed. It is a **registry, not a scraper**, because this repo keeps ~22 superseded figures on purpose and a scraper would push us to delete our own history; superseded numbers are allowed on lines that mark themselves as history, and **every exemption is printed**.
