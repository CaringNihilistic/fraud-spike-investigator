...
| **Net protected value** | **₹10.57L** on the held-out test slice, after 617 human reviews costed at ₹50 each |
...
| **Engineering** | **57 tests**, no network or credentials needed · one-command demo · **24 logged failures** with root causes |
...
One command runs it: `python run_demo.py` (trains, serves, replays 13,987 transactions through the real pipeline). **57 tests pass with no network and no credentials.**

| Metric — temporal held-out test slice, synthetic data (labeled as such) | Value |
|---|---|
| PR-AUC | **0.945 ± 0.007** across 5 independent worlds |
...
- **Risk fusion changes 0 of 13,987 decisions — and we say so.** Kept for auditability, a reachable fail-safe, and headroom for a weaker model; not claimed as a metric win.

## 4. Failure recovery

**24 logged failures** in `CLAUDE.md`, every one caught by *measuring a claim* rather than re-reading code. The five that matter most:
...
**Audit 2 — was the *model* reading our answer key?** Same attack, one layer down (`python -m src.models.leakage_probe`). It found that **two features reproduce the entire 22-feature model**: `customer_age_days` + `amount_dev_ratio` score 0.9328 against the full pipeline's 0.9344. The cause is our generator — attack accounts are created *on the attack day*, giving median account ages of 1–6 days against a legitimate baseline of 216. **This forced us to retract this submission's earlier claim that entity/graph features were the source of the lift.** What it does *not* touch, each checked rather than assumed: train/test hygiene is intact, and every merchant-level result stands — the flash sale cannot be explained by account age, because its customers are *older* than baseline (231 vs 216 days). What entity features still measurably buy: at the deployed threshold they cut legitimate ₹ wrongly blocked from **₹68,319 to ₹5,901**.
...
1. **The simulator encodes the label into two features**, so the headline PR-AUC overstates real detection ability (Audit 2). The fix — regenerating attack accounts with realistic ages — changes every number here, so it is documented rather than rushed.
2. **The merchant-level system is validated on synthetic data only.** Public data validates the *transaction-risk* component; neither dataset we evaluated has a merchant column.
3. **Our entity graph is unvalidated on public data.** The two datasets we evaluated do not expose the persistent entity relationships needed to test it — IEEE-CIS `DeviceInfo` is a device *type* ("Windows" is 40.2% of rows), not a fingerprint, and it has no account identifier. We ran the experiment and publish the rows, but do not present them as evidence either way, because they do not measure the hypothesis. Independent support does exist: Vesta's own entity-counting features are the single largest contributor to IEEE-CIS performance (+0.352).
4. **One legitimate-spike scenario.** The flash sale is the only benign volume event tested; the EWMA baseline has no seasonality term.
5. **The review queue may not be staffable.** 44.1 cases per 1,000 transactions is priced at ₹50 each but never checked against whether the analysts exist.
6. **Metrics carry a range, over one generative process.** PR-AUC 0.945 ± 0.007 across five worlds measures sampling variance, not real-world variance. Differences below ~±0.02 should not be treated as real.
7. **The agent's reasoning is the weakest component.** 8/13 correct cause; on quiet merchants it has twice contradicted its own evidence. It reads instrument *novelty* as evidence against card testing when that is the signature of it (failure 22, logged and deliberately unfixed — the design is frozen and the fix must come from evidence, not prompt hints). Output varies run to run. It is advisory only and cannot act.
8. **Risk fusion changes 0 of 13,987 decisions.** Retained for architecture, not metrics.
9. **Not production hardened.** Single process, in-memory state (lost on restart), no idempotency. Every mutating endpoint requires a shared write key, but that is a single-tenant gate, not identity — an analyst override carries no attributable actor.
